...
class DatabaseSeeder(private val database: PostgresDatabase) {
    suspend fun seed() {
        database.transaction { connection ->
            ...
            connection.backfillUserJoinedPlansAndCompletedWorkouts()
        }
    }
}
...
private fun Connection.backfillUserJoinedPlansAndCompletedWorkouts() {
    prepareStatement(
        """
    ...
    prepareStatement(
        ...
        ON CONFLICT (user_id, plan_id, workout_id) DO NOTHING
        """.trimIndent()
    ).use { it.executeUpdate() }
}
